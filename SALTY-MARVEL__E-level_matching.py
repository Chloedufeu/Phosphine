"""
This script is used to match E-levels between the MARVEL and SALTY datasets based on quantum numbers (v1, v2, v3, v4, J, K) and energy values. It identifies perfect matches using the listed quantum numbers and energy proximity, handles near-matches, detects conflicts where multiple MARVEL lines match the same SALTY line, and generates a histogram of energy differences for perfect matches. It outputs matched data and lists unmatched MARVEL entries.
...
no_exact_match_counter = 0 
no_match_within_Erange_counter = 0  
rows_with_perfect_match_counter = 0  
salty_conflict_counter = 0    
marvel_conflict_counter = 0  
no_match_within_Erange_list = []  
matched_salty_lines = {}   
...
def combine_rows(Marvel_row: pd.Series, match_data: pd.DataFrame, Marvel_energy_col: int, SALTY_energy_col: int, SALTY_Jtag_col: int, SALTY_Ktag_col: int, SALTY_v1tag_col: int, SALTY_v2tag_col: int, SALTY_v3tag_col: int, SALTY_v4tag_col: int) -> pd.Series:
    ...
    if filtered_data is None:
        ...
        no_match_within_Erange_list.append(Marvel_row.tolist())  
    ...
    if filtered_data is not None:
        ...
        # If multiple exact matches were found, choose the one with the smallest energy difference
        if exact_matches:
        ...
        # If no exact match, calculate energy difference
        ...
        # Print the Marvel row currently being matched
        print("Marvel row:")
        print("v1,v2,v3,v4,  J,  K,L3,L4, energy")
        print(Marvel_row[[0, 1, 2, 3, 6, 7, 8, 9, 14]].tolist())
...
def main() -> None:
    ...
    Marvel_filename = "Output_MARVEL_E-levels.txt"
    SALTY_filename = "Original_SALTY_E_energy_states.txt"
    out_filename = "MARVEL-SALTY_matched_A-levels.txt"
    
    Marvel_energy_col = 14  
    SALTY_energy_col = 1   
    SALTY_Jtag_col = 3     
    SALTY_Ktag_col = 6     
    SALTY_v1tag_col= 9      
    ...
    print("Matching data")
    # Apply the `combine_rows` function 
    ...
    if not out_df.dropna().empty:
        energy_diff_col_index = out_df.shape[1] - 1 
    ...
    # Save the matched rows to a file
